...
describe '#537 enum values spec' do
  let(:app) do
    Class.new(Grape::API) do
      namespace :issue_537 do
        class Issue537Spec < Grape::Entity
  ...
  let(:property_default) { subject['definitions']['Issue537Spec']['properties']['enum_property_default'] }
  specify do
  ...
  let(:own_format) { subject['definitions']['Issue537Spec']['properties']['own_format'] }
  specify do
